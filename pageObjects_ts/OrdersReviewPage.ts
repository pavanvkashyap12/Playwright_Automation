...
export class OrdersReviewPage {
    ...
    async searchCountryAndSelect(countryCode: string, countryName: string) {
        ...
        for (let i = 0; i < optionsCount; ++i) {
            ...
            if (text.trim() === countryName) { // Here we have to declate text first and assign it any if assigned string it will say what if it is null so to handle null we can assign any
